...
provides_tmp=$(mktemp --tmpdir provides.XXXXXX)
...
sum_tmp=$(mktemp --tmpdir sum.XXXXXX)
...
sum_current_tmp=\$(mktemp --tmpdir sum.current.XXXXXX)
if test -z "\$sum_current_tmp" ; then
	echo "unable to make a temp file"
	exit 11
fi

sum_package_tmp=\$(mktemp --tmpdir rpm.sum.package.XXXXXX)
